...
exports.createServer = function createServer(options) {
  ...
  //custom code adding custom headers 
   proxy.on('proxyReq', function(proxyReq, req, res, options) {
    

    // get cookies
    const cookies={}
   req.headers.cookie?.split(";").map((el)=>{
    return el.trim()
     }).forEach(el=>{
     const coki=el.split("=")
     cookies[coki[0]]=coki[1]
    })
     

     
   console.log(req.headers.cookie)
    //set referer header obtained from the cookie
     console.log(cookies.referer)
     if(cookies.referer)
     {

       proxyReq.setHeader('referer', decodeURIComponent(cookies.referer));
